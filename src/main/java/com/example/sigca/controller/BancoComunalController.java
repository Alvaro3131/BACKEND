...
public class BancoComunalController {
	...
	//Funciona
			...
			public ResponseEntity<List<BancoComunal>> listarBancoDistrito(@PathVariable("id") int id){
				try {
					List<BancoComunal> list = new ArrayList<>();
					list=bsi.buscarDistrito(id);
					if (list.isEmpty()) {
			...
			@GetMapping("/sede/{id}")
			public ResponseEntity<List<BancoComunal>> listarBancoSede(@PathVariable("id") int id){
				try {
					List<BancoComunal> list = new ArrayList<>();
					list=bsi.listarBancoComunalSede(id);
					if (list.isEmpty()) {
						return new ResponseEntity<>(HttpStatus.NO_CONTENT);
					}
					return new ResponseEntity<>(list,HttpStatus.OK);
